...
# Add CORS middleware with proper configuration for Vercel
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
...
async def health_check():
    return {
        ...
        "ready_for_chat": is_initialized,
        "chains_loaded": bool(qa_chain_en and qa_chain_hi),
        "vector_db_ready": bool(vector_db)
    }
...
async def not_found_handler(request, exc):
    return {"error": "Endpoint not found", "available_endpoints": ["/", "/health", "/chat", "/tts", "/clear-memory", "/system-info"]}

@app.exception_handler(500)
async def server_error_handler(request, exc):
    logger.error(f"Server error: {exc}")
    return {"error": "Internal server error", "message": "Please check server logs"}
